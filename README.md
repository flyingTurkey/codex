...
当前第 14 轮工程验收状态为 `PASSED`：Feed、搜索、日报、收藏、专题、下载和详情已在一次受控切换中统一使用 Event 身份；旧 Item 仅保留不可变映射、308 页面跳转和有期限的只读 API 兼容。第 11 轮生产证据状态仍为 `BLOCKED`：真实业务金标、连续运行窗口、生产 PITR 和真实告警路由等证据尚未完整。仓库不应被表述为生产就绪。
