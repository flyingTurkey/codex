...
DeepSeek 不可用时题录和原文链接仍可用，本地规则还可提取带原文定位的标题、发布者、明确日期、整数人数和带币种金额；平台不会补写摘要。自动内容属于普通资讯，精选仍由现有管理决策产生。详见[证据事实与 AI 判断](docs/user-guide/personal-evidence-facts.md)。
...
当前 AI 纵向切片只允许规范来源 `GOV-003` 的一个固定交通运输部公开 PDF。PERS-06 已将其从 `WAITING_CLAIM_REVIEW` 改为抽取后自动证据门禁；模型输出本身不具有事实或发布授权，只有服务端门禁通过的 Claim 才能进入个人最小投影。
