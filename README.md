...
当前第 14 轮独立验收状态为 `NOT_COMPLETED`：迁移、别名、308/API 兼容和工程门禁成立，但默认普通用户 Feed/详情/搜索仍由 `srbg_api_login` 读取业务表，尚未切换到 `srbg_projection_reader_login` 的 `published_v1`；搜索、日报、收藏、专题和反馈消费者也仍保留并查询 `item_id` 主路径，因此不能宣称 Event 已成为唯一用户身份。第 11 轮生产证据状态仍为 `BLOCKED`：真实业务金标、连续运行窗口、生产 PITR 和真实告警路由等证据尚未完整。仓库不应被表述为生产就绪。
