...
## 2026-08-01 (Phase 5 formal one-day operation — preparation)

- Candidate `5e2ae370d303af4b0dd688a2e989957513e21622` passed fast, PR, quality, its sole release run, and same-SHA GitHub Actions run `30709429791`. The required isolated real-model evaluation then correctly rejected `ai01-extract-v2`: three evidence mappings were exact, but one combined an issued evidence ID with the next source block and locator because the evidence catalog and document were separate correlation surfaces. Formal PostgreSQL remains at `0058` with all Writers stopped. Added immutable `ai01-extract-v3` at forward-only `0060_phase5_extract_prompt_v3`; each untrusted source block is now one atomic `evidence_id`/block/locator/text record, while the Schema, local evidence validator, publication authority, model, source policy, Compose, and `dev-lite` remain unchanged.
- Candidate `afee39e44911e6b18b9c4512a64ec2e4b3562795` passed fast, PR, and quality gates, but its sole release run stopped at `pip-audit` after PyPI closed a TLS connection while the auditor queried the unpublished editable workspace distribution `srbg-api`. The release is recorded as failed, not retried or reported green. The security target now uses the auditor's supported `--skip-editable` boundary, so the three local packages are explicitly excluded while every installed third-party dependency remains audited; pnpm and Trivy HIGH/CRITICAL checks are unchanged. A focused test locks that boundary, and the repaired security target passes.
- The repaired `0058` candidate and API ACL passed its sole release gate and same-SHA CI, and formal PostgreSQL was backed up, restored in isolation, migrated, and started with the full content profile. A second bounded article reproduced the two-attempt EXTRACT rejection before any publication. The failure exposed a first-principles contract gap: DeepSeek receives only `json_object`, while the EXTRACT request supplied neither the extraction Schema nor the server-issued evidence-ID catalog, so the model could not produce evidence identifiers that the non-bypassable local gate would accept. The run was fail-closed as `FORMAL_EXTRACTION_PROMPT_DEFECT`; the SourceStream, schedule, and Writers were stopped. Added versioned `ai01-extract-v2` registration at forward-only head `0059_phase5_extract_prompt_v2` and a bounded extraction envelope containing the unchanged Schema plus exact server-issued evidence IDs, block IDs, and locators. Evidence validation, publication authority, source policy, model, Compose, and `dev-lite` remain unchanged; new candidate gates, isolated prompt evaluation, a fresh pre-0059 formal backup/restore, migration, and an independent eight-hour run remain required.
- The first bounded formal pre-run admitted exactly one `RES-004`/`CJHT_CURRENT_ISSUE` SourceStream and fetched one real official journal article. Discovery, fetch, parse, and `AUTO_ACCEPTED` qualification succeeded, while both unchanged EXTRACT attempts were explicitly rejected as `MODEL_OUTPUT_REJECTED`; the document remained unpublished with an auditable reason. The same run also exposed a production ACL defect: the API technical-exception reconciler reads `ai_compensation_run_v2`, but `srbg_api_role` had no `SELECT`, causing a visible five-second reconciliation error loop. Writers were stopped, the run was marked `FAILED/FORMAL_API_ACL_DEFECT`, and immutable NO_GO evidence was retained. A minimal forward-only `0058_phase5_technical_exception_acl` grants only that missing read and no write privilege; the existing retry function, publication authority, model, Prompt, Schema, source policy, Compose profiles, and `dev-lite` semantics are unchanged.
- Started Phase 5 from the Phase 4 live code candidate `9250862f9d09b873296779558ee3bc27f65e0523`, explicitly excluding the documentation-only successor. Formal preflight found the already-applied Phase 2C sibling `0055_crossref_metadata_admission`, including one durable Crossref-provenance stream configuration, rather than the documented `0054` baseline. Added a forward-only `0057_phase5_formal_reconciliation` merge with a non-executable historical anchor: it preserves that provenance schema and data, applies the Phase 3 authority and Phase 4 controlled-handoff branches, and yields one head without cherry-picking or copying the Phase 2C connector migration. Fresh `0054` and reconstructed formal-sibling paths both pass disposable replay; durable legacy provenance blocks destructive downgrade and formal rollback remains application rollback plus a newly verified backup.
- Added the explicit `phase5-formal-one-day` live verification profile. It consumes append-only periodic samples from one controlled run and rechecks formal PostgreSQL plus loopback `/all`, `/api/v2/feed`, and Event detail surfaces. PASS locks revision and all fixed controlled-run limits, requires an eight-hour window aligned to the run, exactly one target SourceStream and no other-source fetches, only R1/R2 `INDUSTRY_UPDATE` publication, at least one parsed/AI-successful/published Event with accepted-claim evidence links, zero duplicate/silent-failure/permanent-hang counts, explained unpublished documents, budget compliance, and observed Worker restart recovery. All three reader surfaces must contain the Event; the snapshot query is compiled against a disposable 0057 PostgreSQL schema. The profile is read-only apart from writing acceptance evidence; `dev-lite` and all Compose profiles are unchanged. Formal backup, migration, source activation, one-day operation, and final `check-live` remain pending and are not claimed here.

## 2026-07-31 (Phase 4 controlled real Event acceptance — resumed)

- Candidate `022f7f19bec3e7ec2338b4d8e27ea6f5a256052a` passed all prescribed gates and same-SHA CI, then completed the narrowed real CJHT acquisition through raw object and DocumentVersion with zero model calls. The first `/sources` projection attempt stopped only because the acceptance harness supplied `SourceRegistryService` an object without its constructor-required `metrics` attribute. A metrics-bearing read-only acceptance adapter fixes that harness seam without changing production source, UI, model, publication, or database behavior; the immutable failed run remains `NO_GO` pending the repaired-SHA rerun.
- The Owner explicitly narrowed Phase 4 from a publication/Event closure to one real-source display closure. The guarded profile is now `phase4-source-display`: one admitted CJHT SourceStream performs bounded list/detail acquisition, persists the raw object and DocumentVersion, and must appear through the existing source-registry Interface consumed by `/sources`, including last successful fetch, last content discovery, and 1/1/0 run counts. The profile makes zero model calls and no longer loads a DeepSeek secret or creates an AI pipeline, claims, publication, Feed item, Reader projection, or replay. No production source, qualification, publication, or UI rule was weakened; stop/drain and isolated PostgreSQL/Redis/MinIO teardown remain mandatory.
- Frozen candidate `0394ae8817f58ab52ceb086031c341e11777fb20` passed `check-fast`, full `check-pr`, its sole `check-release`, and same-SHA CI before the fixed CJHT document was rerun in a fresh isolated live project. List/detail fetch and production qualification passed, but both unchanged-Schema EXTRACT attempts were rejected; the stable rejection-code repair prevented the previous database-finalizer masking, while the acceptance correctly stopped on incomplete provider usage/cost evidence with `AI_COST_EVIDENCE_MISMATCH`. The queue drained and every scoped resource was removed. The phase-wide conservative bound is now at most 145,583 micro-USD (about RMB 0.99), leaving less than one model-call reservation, so no further live request is authorized. Phase 4 remains `NO_GO`; Phase 5 and formal database writes remain unstarted.
- Candidate `a62c2e0c141a60d1f61a4d9b4813f9b85d58eaa1` passed all prescribed gates and same-SHA CI, then fetched the fixed CJHT list/detail, persisted raw evidence, and obtained a real `AUTO_ACCEPTED` `INDUSTRY_UPDATE` qualification. Three DeepSeek HTTP calls followed, but two EXTRACT outputs failed the unchanged local Schema gate; the exhausted repair path returned a human-readable rejection message as a database reason code, so the legacy finalizer rejected the lowercase/spaced value and masked the real model-output rejection. The AI process now returns the existing stable `MODEL_OUTPUT_REJECTED` code; no Schema, repair count, evidence or publication rule is loosened. Because only 534 micro-USD of the three calls is exactly reconstructable, the next cap is reduced to 28,000 micro-USD so the phase-wide conservative worst case remains below RMB 1.
- After repaired SHA `561c8c96ed170519610a8e47b29fd8df6c899b51` passed all local/release gates and same-SHA CI, two isolated same-document live attempts stopped before source response or model use. Their coarse persisted `DNS` label was narrowed to intermittent `httpx2.ConnectError` after successful independent DoH, pinned-IP SOCKS, TLS, and a later traced HTTP `200`; both runs cost zero and fully drained. The acceptance profile now permits exactly one bounded transport retry with three total physical request slots while retaining one source, one document, one request per minute, the 25-minute deadline, 40,000-micro-USD model cap, and every SourceAdmission/SSRF/evidence/publication gate. No production retry policy, VPN, system network, formal data, or Phase 5 state changed.
- Candidate `6e38078d70bb5090f0cc095085e78939d2672a40` passed `check-fast`, full `check-pr`, its one exact-SHA `check-release`, and same-SHA CI. Its isolated `CJHT_CURRENT_ISSUE` live run fetched and persisted the frozen journal article but stopped before model use because the generic HTML parser discarded the standard scholarly abstract metadata and the title alone did not contain a recognized engineering-activity term. The focused repair admits only standard title/abstract/publication-date metadata into the existing untrusted-document blocks; it changes no qualification term, source boundary, model, Prompt, Schema, budget, evidence gate, or publication authority. The failed evidence remains append-only, cost is zero, and Phase 4 remains `NO_GO` pending repaired-SHA gates and the same-document rerun.
...
- Candidate `6e4e84a0e1c684717867cdac1a298a7757cf89f1` passed the complete repair gates and same-SHA CI, but two bounded live attempts received HTTP `521` from the fixed CCCC collection URL, including after the authorized FlClash restart. Both stopped before document discovery or model use. The latest teardown removed every isolated resource, but its database-level drain evidence was masked by an idempotency defect in the stop path, so Phase 4 remains `NO_GO` and Phase 5 remains unstarted.
- Hardened the acceptance harness without weakening source or publication gates: each run now writes a uniquely named evidence file with exclusive-create semantics, the previously recorded SourceStream UUID is fixed before network I/O, and stop/drain first appends a deterministic disabled owner intent before pausing the schedule and stopping the isolated runtime. Three focused regressions were red before the minimal repair and now pass.
- Candidate `44aafabc7d827d4d0d8caf3cecd8876003c94731` passed local `check-fast`, full `check-pr`, its one exact-SHA `check-release`, and same-SHA CI. The next isolated live run again received HTTP `521` from the fixed CCCC list URL and stopped before discovery or model use; the repaired stop path proved zero active database tasks and Redis keys, and teardown removed every scoped resource. Phase 4 therefore remains `NO_GO`, the formal database is untouched, and Phase 5 remains unstarted.
- Replaced VPN-controlled system DNS as acquisition authority with a platform-scoped, authenticated RFC 8484 resolver. Source runtime, personal probes, discovery probes, and the pinned paid-search transport now obtain real A/AAAA answers through one configurable DoH endpoint, retain the existing public-IP/redirect/peer pinning checks, and fail closed without system-DNS fallback. No FlClash, Windows network, proxy, route, firewall, hosts, or other-project setting is changed.
- Corrected the fresh-cluster Round 16 ACL replay exposed by same-SHA CI: `retention_execution` is now granted to the migration-owned `srbg_publication_writer` role instead of assuming the deployment-specific `srbg_publisher_login` already exists. Disposable publisher logins inherit the same bounded role, so the publication permission boundary is unchanged and fresh isolated migrations are deterministic.
- Added an explicit application-only SOCKS5 route for VPN-hosted acquisition without changing VPN or system settings. The route accepts only loopback or Docker host-gateway endpoints without credentials, sends only the independently resolved and SSRF-approved public target IP in SOCKS CONNECT, and retains the original hostname for TLS SNI/certificate verification; it is disabled by default and never inherits the system proxy.
- Restored the Phase 4 live harness's production request and response accounting callbacks after the first local-SOCKS attempt proved that an authoritative request-count rejection could mask the underlying bounded transport failure. Failed fetch evidence now includes the persisted `fetch_run.failure_class`; no source rule, proxy behavior, budget, publication gate, or system network setting is changed.
- Made Phase 4 controlled shutdown cancel its exact unleased fetch run through the authoritative scheduling service after source authority is closed. A failed fetch left in `RETRY_WAIT` can no longer be counted as drained merely because disposable resources are later deleted; production retry policy and ordinary runtime behavior are unchanged.
- Candidate `07ddd8e3c41ceb49b244c01635d3e7533efe8c64` proved the repaired shutdown drains database and Redis work to zero, but the fixed CCCC SourceStream again returned HTTP `521` through the application-scoped VPN route. Phase 4 remains `NO_GO`; no document, model call, publication, formal-database write, or Phase 5 work occurred.
- After the Owner authorized replacing the persistently unavailable CCCC stream, current research rejected Sichuan Transport, CSCEC, and XCMG because their authoritative legal evidence is `VERIFIED_RESTRICTED` and must pause under ADR-0005. The guarded live profile now fixes the existing `sany-construction-cases` stream, a new SourceStream UUID, and the one list-discovered `/case/16504.html` document before network I/O. Its official list, detail, robots, and legal-statement pages are currently reachable; the statement permits personal noncommercial use, while every claim remains a manufacturer statement and public full-text/media redistribution remains prohibited. No model or isolated acceptance run was performed while preparing this repair.
- The first replacement-source live attempt stopped before source access when Compose reused the normal stack's `.env` MinIO console port even though the acceptance runner had randomized every data-service API port. The runner now also allocates an isolated `MINIO_CONSOLE_PORT`; it does not stop or reconfigure the existing MinIO, Docker stack, VPN, source, model, budget, or publication path. The failed attempt created no document or model cost and removed all scoped resources.
- Kept the security gate viable on a shared Docker Desktop without stopping other projects: Trivy now uses one scanning goroutine instead of auto-running five while retaining the same 1,311-file Git delivery set, secret and misconfiguration scanners, HIGH/CRITICAL threshold, and nonzero exit on findings. The change was made only after repeated `cannot allocate memory` failures; the unchanged coverage completed with zero findings.
- Bounded every Compose command issued by this repository to one concurrent Docker Engine operation. This prevents Compose v5/Buildx Bake from exhausting a shared Docker Desktop during the required gates while leaving Docker's global configuration, other projects, FlClash, Windows networking, images, volumes, and test coverage unchanged.
- Made Windows personal-data readiness use the responsive Docker Engine to verify an already attached VHD and restore observability permissions. The system WSL command remains the sole mount authority when attachment is actually required, but an unavailable `wsl -d docker-desktop` control channel can no longer be misreported as missing persisted directories.
- Recorded the remaining Phase 4 local-gate blocker without retaining an unverified workaround: on the shared Docker Desktop, Compose/BuildKit eventually makes the Engine unresponsive during the Web image build even with a repository-scoped single-parallel, 3 GiB builder and bounded pnpm concurrency. The experiment was reverted and its two temporary builders removed; the fixed Sany source, formal database, other projects, FlClash, and Phase 5 were not changed.
- After the Owner authorized stopping only the exact running `overseas-*` container list, fixed candidate `5c9c4d164c4ba8c90104752034d3fb2b481cfa87` passed `check-fast`, full `check-pr`, its one exact-SHA `check-release`, and same-SHA CI without changing FlClash, protected databases, volumes, or system networking. The first Sany live run fetched the fixed detail successfully but honestly stopped at an acceptance-only legacy `fetch_record` join before model use; append-only evidence records zero model cost and a complete database, Redis, container, volume, and network drain. A focused red/green repair removes only that unused legacy dependency; Phase 4 remains `NO_GO` pending full repaired-SHA gates and the same-document rerun, and Phase 5 remains unstarted.
- Candidate `f90e83ff3dea7c263f7e54e3e21b6dcbe9794126` passed the required local gates, its one exact-SHA release gate, and same-SHA CI. The repaired same-document live run persisted the raw object and DocumentVersion and made two real DeepSeek calls, then honestly stopped before publication because the acceptance report queried nonexistent denormalized AI registry columns. The next focused repair reads prompt/schema/model versions through their stored foreign keys and snapshots non-content token/cost metadata after every response. Since the failed evidence cannot truthfully reconstruct its exact cost, the next controlled-run cap is reduced to 50,000 micro-USD with 12,000-micro-USD per-call reservations, keeping the phase cumulative worst case below RMB 1 at the observed exchange rate. Phase 4 remains `NO_GO` pending repaired-SHA gates and a full same-document rerun; Phase 5 remains unstarted.
- Candidate `b22f0105add2cedd5d0d509e00e611d44f523824` passed the required local/release gates and same-SHA CI, then persisted the same real document without making a model call before an API-role permission failure in the technical-retry coordinator. A zero-network disposable diagnosis proved the Worker role already has the required table ACL; the acceptance harness had rebound the AI repository to the Worker URL but not the coordinator factory. The minimal repair binds both services to isolated Worker engines and changes no migration, role, source, budget, model, production service, or publication gate. Phase 4 remains `NO_GO` pending repaired-SHA gates and the same-document rerun; Phase 5 remains unstarted.
- Candidate `becb925bb1ede775740cdbabf61fbdae47b5b9e3` passed all required gates and same-SHA CI, and the next live run proved the Worker coordinator can append its technical-retry transition. No model request occurred, but the acceptance report then treated an uninitialized zero-call cost-completeness flag as a mismatch. The focused repair initializes complete zero-call/zero-cost evidence and saves pipeline status before consistency checks; it changes no runtime decision, role, source, model, budget, or publication behavior. Phase 4 remains `NO_GO` pending repaired-SHA gates and the same-document rerun; Phase 5 remains unstarted.
- Candidate `31302b2f2fa999ee5415ef7ab3701a2c31f694c3` passed `check-fast`, full `check-pr`, its one exact-SHA `check-release`, and same-SHA CI. The same-document live run then exposed the underlying `CLASSIFYING` technical retry before model use: the source-content handoff copied document and policy authority into the AI run but omitted the fetch run's `controlled_run_id`, so the tightened controlled-cost reservation correctly failed closed. A new linear Alembic head propagates that existing authority through the same narrow handoff command, and live acceptance verifies the binding before any provider request; it does not change the source, budget, model, publication gate, or formal database.
- Updated the migration-chain tests that intentionally pin the repository's current Alembic head after the new controlled-handoff repair made `0056_phase4_controlled_handoff` the sole head. The first repair SHA's `check-fast` stopped only on those stale head expectations; all API tests pass after this mechanical follow-up.
- Added a dedicated current-head migration verifier for the `0055`→`0056` controlled-handoff edge after the first repaired live run proved its Phase 3 verifier intentionally stopped the disposable database at `0055`. Generic isolated/current-head gates and Phase 4 live now replay `0056` upgrade, downgrade, and re-upgrade, while the Phase 3 slice retains its original `0055` verifier.
- Kept an already-reserved controlled source request authorized through its response even when that reservation consumes the schedule's final request slot. Continuous heartbeats no longer confuse exhausted future capacity with revoked authority; the unchanged atomic pre-request reservation still rejects every request beyond the request or byte budget.
- Candidate `cf83f8dd2902a318ceb98b857eea6b3cff9851dc` passed all local/release gates and same-SHA CI, and its isolated live run proved the reserved-request repair by reaching the real model. Two Schema-valid DeepSeek classification calls exhausted the bounded semantic recheck but did not receive automatic qualification, so no accepted claim, publication, Feed item, or Reader projection was created. The run drained all tasks and scoped resources; Phase 4 remains `NO_GO` and Phase 5 remains unstarted.
- After Owner authorization, kept the existing Sany SourceStream and production qualification unchanged while retargeting Phase 4 to the list-discovered `/case/16434.html` rotary-drilling construction case. The live harness now accepts the policy's single bounded semantic recheck, records only the persisted qualification disposition, and still requires `AUTO_ACCEPTED` followed by exactly EXTRACT, SUMMARIZE, and VERIFY; filtered content cannot pass. No model or isolated acceptance run occurred while preparing this minimal repair.
- Candidate `2a6095632a3cf6408fb71d1bfa5e83c0c1ce3e1e` passed all local/release gates and same-SHA CI. Its isolated live run fetched and persisted the fixed `/case/16434.html` document and made two real Schema-valid DeepSeek classification calls costing 728 micro-USD, but the bounded semantic recheck ended in the production disposition `AUTO_FILTERED`. No accepted claim, publication, Feed item, or Reader was created; queue/resource drain passed, the exact forty temporarily stopped `overseas-*` containers were restored, Phase 4 remains `NO_GO`, and Phase 5 remains unstarted.
