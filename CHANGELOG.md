...
- Fixed a latent technical-exception recovery defect exposed by the candidate branch's fresh CI run: repeated Owner recovery now follows the unique `source_content_outbox.pipeline_run_id` binding instead of guessing from historical pipeline timestamps, and projects only a policy-matched `DEAD_LETTER` / `FAILED` technical run. The integration seam deliberately keeps its retry clock behind the handoff wall clock and verifies that re-exhaustion rebinds the existing exception to the actual failed recovery pipeline.
- This result is an isolated candidate-fix `PASS`, not a formal-database migration or production closeout. The Owner authorized local commit, fast-forward integration, repair/retest, and remote candidate push on 2026-07-29; formal-database migration remains a separate action.
