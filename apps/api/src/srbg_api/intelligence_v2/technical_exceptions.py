                ...
                (
                    await connection.execute(
                        text(
                            ...
                            "content.pipeline_run_id FROM automated_qualification_decision_v2 "
                            "decision JOIN document_version version "
                            "ON version.id=decision.document_version_id JOIN document "
                            "ON document.id=version.document_id JOIN source_content_outbox content "
                            "ON content.document_version_id=version.id JOIN ai_pipeline_run run "
                            "ON run.id=content.pipeline_run_id "
                            "AND run.document_version_id=version.id LEFT JOIN "
