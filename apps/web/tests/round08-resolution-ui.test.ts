...
describe('round 08 explainable resolution UI', () => {
  ...
  it('adds hot topics, source comparison and a governed cluster workbench in the shared shell', () => {
    ...
    expect(workbench).toContain('合并理由')
    expect(workbench).toContain("decide('LINK_RELATION')")
    expect(workbench).toContain('relation_type: selected.value.relation_type')
    expect(workbench).toContain("selected.kind === 'EVENT' || selected.kind === 'TOPIC'")
  })
})
