...
test('skip link is first, moves focus to main, and desktop navigation remains tabbable', async ({
  ...
  await expect(page.getByRole('link', { name: 'AI 模型配置', exact: true })).toBeFocused()
  await page.keyboard.press('Shift+Tab')
  await expect(page.getByRole('link', { name: 'Feed 偏好', exact: true })).toBeFocused()
  await page.keyboard.press('Shift+Tab')
  await expect(page.getByRole('link', { name: '技术异常', exact: true })).toBeFocused()
  await page.keyboard.press('Shift+Tab')
  await expect(page.getByRole('link', { name: '我的来源', exact: true })).toBeFocused()
  await page.keyboard.press('Shift+Tab')
  ...
  await page.keyboard.press('Shift+Tab')
  await expect(page.getByRole('link', { name: '行业热点', exact: true })).toBeFocused()
})
...
test('mobile drawer has keyboard focus containment, Escape close, and scroll restoration', async ({
  ...
  await page.keyboard.press('Shift+Tab')
  await expect(dialog.getByRole('link', { name: 'AI 模型配置', exact: true })).toBeFocused()
  await page.keyboard.press('Shift+Tab')
  await expect(dialog.getByRole('link', { name: 'Feed 偏好', exact: true })).toBeFocused()
  await page.keyboard.press('Shift+Tab')
  await expect(dialog.getByRole('link', { name: '技术异常', exact: true })).toBeFocused()
  await page.keyboard.press('Shift+Tab')
  await expect(dialog.getByRole('link', { name: '我的来源', exact: true })).toBeFocused()
  await page.keyboard.press('Tab')
  await expect(dialog.getByRole('link', { name: '技术异常', exact: true })).toBeFocused()
  await page.keyboard.press('Tab')
  await expect(dialog.getByRole('link', { name: 'Feed 偏好', exact: true })).toBeFocused()
