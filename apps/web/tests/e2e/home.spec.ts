...
test('skip link is first, moves focus to main, and desktop navigation remains tabbable', async ({
  ...
  await page.keyboard.press('Shift+Tab')
  await expect(page.getByRole('link', { name: '质量看板', exact: true })).toBeFocused()
  await page.keyboard.press('Shift+Tab')
  await expect(page.getByRole('link', { name: '运行中心', exact: true })).toBeFocused()
  await page.keyboard.press('Shift+Tab')
  await expect(page.getByRole('link', { name: '来源健康', exact: true })).toBeFocused()
  await page.keyboard.press('Shift+Tab')
