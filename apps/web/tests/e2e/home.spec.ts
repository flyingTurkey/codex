...
test('skip link is first, moves focus to main, and desktop navigation remains tabbable', async ({
  ...
  await expect(page.getByRole('link', { name: 'R17观察门禁', exact: true })).toBeFocused()
  await page.keyboard.press('Shift+Tab')
  ...
  await page.keyboard.press('Shift+Tab')
  await expect(page.getByRole('link', { name: '覆盖缺口', exact: true })).toBeFocused()
  await page.keyboard.press('Shift+Tab')
...
test('mobile drawer has keyboard focus containment, Escape close, and scroll restoration', async ({
  ...
  await page.keyboard.press('Shift+Tab')
  await expect(dialog.getByRole('link', { name: 'R17观察门禁', exact: true })).toBeFocused()
  await page.keyboard.press('Shift+Tab')
