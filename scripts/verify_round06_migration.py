"""Replay the Round 06 Alembic boundary on an isolated local database."""
...
def main() -> None:
    ...
    if not _is_loopback(parsed.hostname) or _DISPOSABLE_DATABASE.fullmatch(database_name) is None:
        raise RuntimeError("Round06 migration replay requires an isolated loopback database")

    config = Config("apps/api/alembic.ini")
    command.upgrade(config, "0006_digital_cases")
    command.upgrade(config, "0007_papers")
    command.downgrade(config, "0006_digital_cases")
    command.upgrade(config, "0007_papers")
    print("Round06 migration replay passed: 0006 -> 0007 -> 0006 -> 0007")
