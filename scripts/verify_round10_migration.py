"""Replay the Round 10 Alembic boundary on an isolated loopback database."""
...
def main() -> None:
    ...
    if not _is_loopback(parsed.hostname) or _DISPOSABLE_DATABASE.fullmatch(database_name) is None:
        raise RuntimeError("Round10 migration replay requires an isolated loopback database")
    config = Config("apps/api/alembic.ini")
    command.upgrade(config, "0010_ai_editorial_governance")
    command.upgrade(config, "0011_feed_search_daily")
    command.downgrade(config, "0010_ai_editorial_governance")
    command.upgrade(config, "0011_feed_search_daily")
    print("Round10 migration replay passed: 0010 -> 0011 -> 0010 -> 0011")
