...
- `/` 与 `/selected` 共享“今日精选”当前态；当前态只匹配频道本身或以 `/` 分隔的子路径，不会将 `/selectedness` 等相似前缀误判为当前页；频道链接使用 Nuxt SPA 导航，往返后不发生整页文档导航。
...
- 页面头通过专名状态插槽统一承载状态；根页成功取得版本信息时，将本次真实检查完成时间以 UTC ISO `datetime` 和 `Asia/Shanghai` 可见文本同时呈现，失败态与尚无可靠时间的频道空态不显示更新时间。
