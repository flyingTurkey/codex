...
开发库的 PERS-10 归档验收头为 `0030_pers10_role_archive_repair`，当前应用头为 `0033_controlled_ai_budget_bridge`。归档仍为 53 类、原始 69 行、归档 69 行，manifest 清单汇总 SHA-256 `780f6fd212970588e84d29f9fba169dc5ee5ef4903264cb46f9c831cbb75825c`；其中三个数据库角色的分类哈希为 `a1e0a36ca4bdd7a0cacebb997bd26a19110d4672b6695cad7a0e4f12f9def1d4`。正常业务角色无归档 Schema 使用权。含试点事实的业务库拒绝破坏性降级；回滚应从已验证备份恢复隔离实例，再由 PERS-10 校验逐行及逐类计数和哈希后按 `0030 → 0029 → 0028` 恢复。
...
- 无人值守阶段没有修改用户可见界面；Owner 在审阅唯一推荐稿后明确批准正式实现。首页、来源中心和详情页仍只读取个人链路与 PublicationService 投影，不恢复企业审批行为。
- 固定五源运行按版本化例外策略正式重判为 `PASS`（3/5）；该例外不改变默认 4/5 门槛。应急管理部和交通运输部信息公开入口仍因客户端跳转被安全拒绝，候选替换必须先完成新有界 Probe。
- `0033` 已补齐受控 AI 双账本原子费用门禁，但当前只有 `test` 环境激活记录；生产 AI 专项仍未完成，不阻塞只读界面审查，但在正式界面实现前必须真实通过或由 Owner 明确维持降级。
- 原始试点报告因画像回调失联判为失败并保持不可改写；最新无联网正式重判报告 SHA-256 为 `DA15762E9B7A7F32EF1B73DC0352C587A7105CCAD9318F97AA5BC2311A5C0ABD`。

## 界面审查准入复核

进入只读界面审查前已修复两项数据完整性缺口：真实个人证据信号详情不再因缺少旧发布投影返回 500；同一事件的 metadata 与个人信号只展示一次。72 条未标记的保留测试域名记录已在备份后显式标为 `FIXTURE_TEST`，正常来源和 PublicationService 投影均按状态隔离。自动发现保持关闭。

当前真实样例详情返回 `published_at=2026-07-07` 和 title 两条 accepted claims，对应 Evidence IDs `019f75c0-e4e4-75d8-9f18-536a0932202f`、`019f75c0-e4df-730a-83d4-cb8ef66c68be`，定位均为 `PDF_TEXT`，内容 SHA-256 均有效。原推荐稿位于 `D:\SRBGData\reports\ui-review\core-pages-20260719T0225Z`；Owner 确认后完成正式界面实现，浏览器截图位于 `D:\SRBGData\reports\ui-review\implemented-20260719`，设计对照结论见仓库根目录 `design-qa.md`。

## 2026-07-19 正式界面验收

- 首页真实显示 8 条个人情报、4/50 健康来源、采集投影状态和已关闭的自动发现；没有生成演示分数或伪造 AI 状态。
- 来源中心真实显示 50 个来源、4 个健康来源、3 个异常来源和 0 个当前运行来源；添加 URL 保持一个输入和一个主要动作，自动发现设置默认折叠但仍可键盘访问。
- 真实国务院政策详情首屏显示两条证据事实、两个 Evidence ID、`PDF_TEXT` 定位和两个内容哈希；点击“查看证据定位”会打开服务端 Event 投影的证据抽屉，并显示完整哈希及关联 accepted claim。
- 字体使用权威 Noto Sans SC / Source Han Sans SC 字体栈，窗格渐变只引用现有低饱和设计令牌；点击反馈为 140–180ms，系统请求减少动态效果时禁用非必要动效。
