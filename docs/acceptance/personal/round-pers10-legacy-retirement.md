...
迁移头为 `0029_legacy_governance_retirement`。迁移在单一 PostgreSQL 事务中将旧记录规范化为 JSON，计算逐行 SHA-256 和逐类汇总 SHA-256，验证数量与哈希后才把旧关系移入只读 `legacy_governance_archive`。归档 Schema 对 `PUBLIC` 及应用角色撤权，写操作由数据库触发器拒绝。

开发库实际归档 52 类、原始 66 行、归档 66 行；manifest 清单汇总 SHA-256 为 `c977659c5cfae2355c9b879fcc0bc5b10f9e8bf7bbbb25aa417457461e780274`。非零类别如下：
...
`make personal-migration-test` 使用独立 PostgreSQL 集群执行 `0028 → 0029 → 损坏归档降级拒绝 → 修复 → 0028 → 0029`。专项测试同时验证旧 API 不进入 OpenAPI、契约只保留 owner、业务代码不读取归档、旧审核触发器和旧角色正确退场。
...
- `make test`：945 通过、25 跳过；UI 53/53、Web 单元 90/90。
...
- `make personal-migration-test`：15/15，通过独立 PostgreSQL 的 `0028 → 0029 → 损坏降级拒绝 → 修复 → 0028 → 0029`；30+30 固定评估达标。
...
- 旧管理 API 四个代表路径均为 HTTP 404；最终迁移头为 `0029_legacy_governance_retirement`，退场角色剩余数为 0，应用角色归档 Schema USAGE 均为 false。
