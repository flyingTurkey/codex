...
- Current integration gate: `autonomous-content-integration-test` runs #41's isolated PostgreSQL, Worker/evidence and PublicationService path for both `SAFETY_INTELLIGENCE` and `INDUSTRY_UPDATE` FULL machine-organized projections. It does not claim to restore the retired enterprise Safety Case lifecycle or content-object persistence; Round 04 immutable fixtures remain covered by offline replay.
