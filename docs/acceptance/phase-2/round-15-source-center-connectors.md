...
## 2026-07-17 治理元数据保存反馈回归

- 复现现象：治理元数据表单依赖浏览器原生必填/格式校验，校验不通过时提交事件不会触发；页面又没有监听 `invalid`，因此用户点击后看不到任何解释。服务端错误原先只显示在抽屉背后的页面顶部，也会表现为“无反应”。
- 修复结果：抽屉内显示字段校验或服务端失败原因；保存成功后关闭抽屉并显示独立成功状态。当前登录用户 ID 和三个多选框的填写要求在表单旁可见，“追加双维评估”按钮及其独立流程保持不变。
- 回归覆盖：Web 单元测试锁定三类提示文案；Playwright 覆盖空原因拒绝、补齐原因后 PUT 成功、成功状态可见，以及“追加双维评估”继续可见。
