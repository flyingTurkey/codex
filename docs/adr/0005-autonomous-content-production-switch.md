...
The real content Worker uses the immutable `qualification-policy-2.2.0` bundle after raw-first storage, current-version validation, SourceAdmission, public-network safety, security and budget authorization. Source-content bridge runs remain `LIVE`; a `SHADOW` run may append a decision but is terminal before Item, claim or Feed materialization. Its rule, Prompt `autonomous-classify-2.7.0`, and Schema `autonomous-classify-output-2.0.0` identities match the frozen replay seam. Production and replay render the same system and bounded user-Prompt templates, including the strict Schema, issued evidence-locator allowlist, untrusted-document wrapper and independent re-adjudication instruction. Migration 0049 fails if an existing registry version has different bytes. Deterministic exclusions run before the model. Other candidates use the strict autonomous Prompt and Schema; a rule/AI conflict receives exactly one semantic recheck. Every terminal result is appended to `automated_qualification_decision_v2` with the exact policy identity. New runs do not read Owner Gold and `OWNER_OVERRIDE_GO` cannot authorize them.
