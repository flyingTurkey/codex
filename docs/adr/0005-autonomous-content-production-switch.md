...
Hard public-network, access-control, malicious-file, credential and explicit legal prohibitions remain non-bypassable. The authoritative assessment writer and closeout verifier use the production SourceAdmission verdict: `BLOCKED`, `DENIED`, `DISALLOWED`, `FORBIDDEN` and `RESTRICTED` robots, terms or copyright results pause, while absent or genuinely unknown metadata permits bounded collection. The assessment writer is append-only and cannot change `source.runtime_state`; runtime transitions remain the responsibility of the independently authorized SourceStream controller. HUMAN_OWNER Gold, soft-yield observations and qualification leakage grant no admission authority. Leakage remains observable input to policy iteration. Rate, budget, timeout and circuit-breaker failures pause or retry through the existing technical path rather than becoming semantic rejection. Operational exception UX, suppression UX and policy optimization remain Issues #43–#46.

## Consequences

Migration `0049_autonomous_content_switch` follows 0048, registers the production Prompt/Schema, preserves `INDUSTRY_UPDATE` as an independent compatibility storage type/channel, and grants the Worker only append access to policy and decision facts. Migration `0050_autonomous_handoff_state_order` then makes same-timestamp document lifecycle ordering deterministic, prioritizing terminal security failures over readiness and readiness over intermediate states instead of relying on the random suffix of UUIDv7 identifiers. Its downgrade restores the preceding function definition and its isolated replay is verified in both directions. 0046/0047 remain unchanged history.

Rollback from 0049 is allowed only before the production policy has durable decisions or industry rows; otherwise it fails closed. A new document version receives a new decision and the existing invalidation/outbox path withdraws stale claims and projections.
